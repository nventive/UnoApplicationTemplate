...
public sealed partial class FormattingTextBoxBehavior
{
	...
	{
		var nativeView = (textbox as DependencyObject).FindFirstChildA<SinglelineTextBoxView>();
		if (nativeView == null)
		{
			if (typeof(FormattingTextBoxBehavior).Log().IsEnabled(LogLevel.Error))
			{
				if ((textbox as DependencyObject).FindFirstChildA<MultilineTextBoxView>() != null)
				{
