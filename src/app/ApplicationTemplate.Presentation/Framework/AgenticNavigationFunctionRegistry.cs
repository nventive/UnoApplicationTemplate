...
public class AgenticNavigationFunctionRegistry
{
	...
	private async Task<string> NavigateToPageAsync(JsonDocument args, CancellationToken ct)
	{
		try
		{
			...
			if (!AgenticToolExecutor.ResponseHelpers.TryGetStringParameter(args, "pageName", out var pageName))
			{
				return AgenticToolExecutor.ResponseHelpers.Error(
					"Missing required parameter: pageName. Available pages: DadJokes, Posts, Settings, UserProfile");
			}

			// Extract optional parameters
			AgenticToolExecutor.ResponseHelpers.TryGetBoolParameter(args, "clearStack", out var clearStack, false);
			...
			switch (normalizedPageName)
			{
				...
				case "home":
					...
					return AgenticToolExecutor.ResponseHelpers.Success(
						"Navigated to DadJokes page",
						new { page_name = pageName });
				...
				case "posts":
					...
					return AgenticToolExecutor.ResponseHelpers.Error(
						"Posts page navigation not yet implemented in your navigation structure");

				case "settings":
					// Example: Navigate to settings
					// await _sectionsNavigator.Navigate(ct, () => new SettingsPageViewModel());
					return AgenticToolExecutor.ResponseHelpers.Error(
						"Settings navigation requires ViewModel instantiation. Update this method to include your SettingsPageViewModel.");

				case "userprofile":
				case "profile":
					// Example: Navigate to user profile
					return AgenticToolExecutor.ResponseHelpers.Error(
						"UserProfile page navigation not yet implemented. Update this method to include your UserProfilePageViewModel.");

				default:
					return AgenticToolExecutor.ResponseHelpers.Error(
						$"Unknown page name: {pageName}. Available pages: DadJokes, Posts, Settings, UserProfile");
			}
		}
		catch (Exception ex)
		{
			_logger.LogError(ex, "Error in NavigateToPageAsync");
			return AgenticToolExecutor.ResponseHelpers.Error($"Navigation error: {ex.Message}");
		}
	}
	...
	private async Task<string> GetCurrentPageAsync(JsonDocument args, CancellationToken ct)
	{
		...
		catch (Exception ex)
		{
			...
			return AgenticToolExecutor.ResponseHelpers.Error($"Error getting current page: {ex.Message}");
		}
	}
	...
	private async Task<string> GoBackAsync(JsonDocument args, CancellationToken ct)
	{
		try
		{
			...
			{
				return AgenticToolExecutor.ResponseHelpers.Error("No active stack navigator");
			}

			if (activeStackNavigator.State.Stack.Count <= 1)
			{
				return AgenticToolExecutor.ResponseHelpers.Error("Cannot go back, already at root page");
			}

			await activeStackNavigator.NavigateBack(ct);

			return AgenticToolExecutor.ResponseHelpers.Success("Navigated back successfully");
		}
		catch (Exception ex)
		{
			_logger.LogError(ex, "Error in GoBackAsync");
			return AgenticToolExecutor.ResponseHelpers.Error($"Back navigation error: {ex.Message}");
		}
	}
	...
	private async Task<string> OpenSettingsAsync(JsonDocument args, CancellationToken ct)
	{
		try
		{
			...
			return AgenticToolExecutor.ResponseHelpers.Error(
				"Settings page navigation requires ViewModel factory implementation. Update OpenSettingsAsync in AIAgentNavigationFunctionRegistry.cs");
		}
		catch (Exception ex)
		{
			_logger.LogError(ex, "Error in OpenSettingsAsync");
			return AgenticToolExecutor.ResponseHelpers.Error($"Settings navigation error: {ex.Message}");
		}
	}
}
